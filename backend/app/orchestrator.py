...
def _distribute_across_days(verified: List[Dict[str, Any]], day_count: int) -> List[List[Dict[str, Any]]]:
    """
    ...
    per_day = max(2, min(4, ceil(len(verified) / max(1, day_count))))  # target between 2..4 per day
...
async def build_itinerary(req: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not candidates:
        ...
        candidates = ["City Museum", "Central Park", "Old Town", "Main Cathedral", "Local Market"]
    ...
    if not verified:
        itinerary["notes"].append("Verification returned no POIs; check API keys, quotas, or raise MIN_RATING.")
