...
async def build_itinerary(req: Dict[str, Any]) -> Dict[str, Any]:
    ...
    budget_value = float(req.get("budget", 1000))           # Total budget (not used in mock calc yet)

    # 1) LLM proposes candidate POIs (UNTRUSTED, best-effort)
    candidates: List[str] = await llm_poi_candidates(req)   # Returns a list of strings (names only)
    ...
    verified = await verify_pois(candidates, city)          # Returns detailed dicts with place_id/lat/lng/rating/url
    ...
    if not verified:                                        # If nothing verified (e.g., quota exhausted)
        itinerary = {                                       # Build minimal itinerary shape
            "trip": {"city": city, "days": days, "currency": currency, "budget": budget_value},  # Trip meta
            "days": [{"date": req.get("start_date", "D1"), "items": []}],                         # Day list with empty items
            "totals": {"lodging": 0, "food": 0, "transport": 0, "tickets": 0, "misc": 0, "currency": currency},  # Totals
            "notes": ["LLM proposed POIs, but verification returned none. Check API keys and quotas."],            # Note
            "uncertainties": ["No verified POIs due to verification failure or empty results."]                    # Uncertainty
        }
    ...
    for p in verified[:3]:                                  # Take at most three to keep map readable
        day0_items.append({                                 # Convert each place into an item
            ...
            "source": {"type": "maps", "place_id": p.get("place_id"), "url": p.get("url")},  # Source attribution
        })

    # If we have items, add transit estimates in sequence
    if day0_items:                                          # Only route if we have at least one item
        day0_items = await route_day(day0_items)            # Enrich items with transport duration/distance

    # Compose the full itinerary structure
    itinerary = {                                           # Build response object
        "trip": {"city": city, "days": days, "currency": currency, "budget": budget_value},  # Trip block
        "days": [{"date": req.get("start_date", "D1"), "items": day0_items}],                # Day list
        "totals": {"lodging": 0, "food": 0, "transport": 0, "tickets": 0, "misc": 0, "currency": currency},  # Placeholder
    ...
    itinerary = await estimate_budget(itinerary, currency=currency)  # Replace totals with mock calculation
