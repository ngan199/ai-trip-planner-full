...
from pydantic_settings import (
    BaseSettings,
)  # Import BaseSettings to declare typed env config
...
class Settings(
    BaseSettings
):  # Define a Settings class that reads from env and .env files
    ...
    google_maps_key: str | None = Field(
        default=None, alias="GOOGLE_MAPS_API_KEY"
    )  # Google Maps API key (optional for local tests)
    ...
    openai_model: str = Field(
        default="gpt-4o-mini", alias="OPENAI_MODEL"
    ...
    llm_order_raw: str = Field(
        default="gemini,openai,anthropic", alias="LLM_ORDER"
    )  # Comma-separated provider names

    @property
    def llm_order(self) -> List[str]:  # Expose provider order as a list
        return [
            p.strip().lower() for p in self.llm_order_raw.split(",") if p.strip()
        ]  # Split, trim, and lowercase items
